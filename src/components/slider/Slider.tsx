...
export default class Slider extends React.PureComponent<Props, State> {
  ...
  private autoPlayTimeoutId: undefined | NodeJS.Timeout | number;
  ...
  get eventObject(): EventObject {
    ...
    const { isNextSlideDisabled, isPrevSlideDisabled } = Utils.getSlideItemInfo(
      this.state
    );
  ...
  slideTo(activeIndex = 0, eventType?: EventType) {
    ...
    if (this.isFadeoutAnimationAllowed) {
      const fadeoutIndex = Utils.getUpdateSlidePositionIndex(
        activeIndex,
        this.state.itemsCount
      );

      const fadeoutAnimationPosition = Utils.getFadeoutAnimationPosition(
        fadeoutIndex,
        this.state
      );
      const fadeoutAnimationIndex = Utils.getFadeoutAnimationIndex(this.state);
      this._handleSlideTo({
        activeIndex: fadeoutIndex,
  ...
  async _handleResize(e: Event) {
    ...
    const nextRootComponentDimensions = Utils.getElementDimensions(
      this.rootElement
    );
    ...
    if (
      ...
      const activeIndex = Utils.getUpdateSlidePositionIndex(
        this.state.activeIndex,
        itemsCount
      );
      const currState = Utils.calculateInitialState(
        { ...this.props, activeIndex },
        this.stageComponent
      );
      const translate3d = Utils.getTranslate3dProperty(
        currState.activeIndex,
        currState
      );
  ...
  _handleTouchmove(e, eventData: EventData) {
    ...
    if (
      ...
      (!this.isTouchMoveProcessStarted &&
        Utils.isVerticalTouchmoveDetected(absX, absY, swipeDelta))
    ) {
      return;
    }
    ...
    let position = Utils.getTouchmoveTranslatePosition(
      deltaX,
  ...
  _handleTouchend(e, { deltaX }: EventData) {
    ...
    if (this.isTouchMoveProcessStarted) {
      ...
      const position = Utils.getSwipeTouchendPosition(
        this.state,
        deltaX,
        touchendPosition
      );
  ...
  async _handleSlideTo({
    ...
    this.slideEndTimeoutId = setTimeout(
      () => this._handleBeforeSlideEnd(eventType),
      animationDuration
    );
  }
  ...
  _handleBeforeSlideEnd = async (eventType?: EventType) => {
    ...
    if (Utils.shouldRecalculateSlideIndex(activeIndex, itemsCount)) {
      const nextIndex = Utils.getUpdateSlidePositionIndex(
        activeIndex,
        itemsCount
      );
  ...
  _handleSlideChange(eventType?: EventType) {
    if (this.props.onSlideChange) {
      const event = eventType
        ? { ...this.eventObject, type: eventType }
        : this.eventObject;
  ...
  async _handleSlideChanged(eventType?: EventType) {
    ...
    if (
      Utils.shouldCancelAutoPlayOnAction(autoPlayStrategy) &&
      this.hasUserAction &&
  ...
  _handleMouseEnter = () => {
    ...
    if (
      Utils.shouldCancelAutoPlayOnHover(autoPlayStrategy) &&
  ...
  _clearAutoPlayTimeout() {
    clearTimeout(this.autoPlayTimeoutId as number);
    this.autoPlayTimeoutId = undefined;
  }

  _clearSlideEndTimeout() {
    clearTimeout(this.slideEndTimeoutId as number);
    this.slideEndTimeoutId = undefined;
  }

  clearTouchendTimeout() {
    clearTimeout(this.touchEndTimeoutId as number);
  ...
  async _setInitialState() {
    const initialState = Utils.calculateInitialState(
      this.props,
      this.stageComponent
    );
  ...
  render() {
    ...
    const wrapperStyles = Utils.getRenderWrapperStyles(
      this.props,
      this.state,
      this.stageComponent
    );
    const stageStyles = Utils.getRenderStageStyles(
      { translate3d },
      { transition }
    );
