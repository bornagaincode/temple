...
export const DotsNavigation = ({
  ...
  const dotsLength = Utils.getDotsNavigationLength(
    itemsCount,
    itemsInSlide,
    autoWidth
  );
  ...
  return (
    <ul className={Classnames.DOTS}>
      {Array.from({ length: itemsCount }).map((item, i) => {
        if (i < dotsLength) {
          const isTheLastDotIndex = Utils.checkIsTheLastDotIndex(
            i,
            Boolean(infinite),
            dotsLength
          );
          let nextIndex = Utils.getItemIndexForDotNavigation(
            i,
            isTheLastDotIndex,
            itemsCount,
            itemsInSlide
          );
          let currentIndex = Utils.getActiveSlideIndex(
            isNextSlideDisabled,
            state
          );
          ...
          const className = Utils.concatClassnames(
            Classnames.DOTS_ITEM,
            isActive,
            isCustom
          );
